...
'''
可以根据兴趣生成全景点路径,且去掉兴趣为零的点，除非经过该点到第三个点的距离更近
'''
    ...
    while iter < itermax:
        ...
        for i in range(numant):
            ...
            for j in range(1, numcity - 1):
                listunvisited = [city for city in unvisited if
                                 interest_weights[city] > 0 or distmat[visiting][city] + distmat[city][end_city] <
                                 distmat[visiting][end_city]]

                if not listunvisited:
                    break

                probtrans = np.zeros(len(listunvisited))
                for k in range(len(listunvisited)):
                    interest_factor = interest_weights[listunvisited[k]] if interest_weights[
                                                                                listunvisited[k]] > 0 else 0.1
...
allspots_ACO(dismat, 218, 133, interests)
